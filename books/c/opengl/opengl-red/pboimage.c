...
#define BUFFER_OFFSET(offset) ((GLvoid *) NULL + offset) // NEW
...
void init(void)
{
   ...
   glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    ...
    glBufferData(GL_PIXEL_UNPACK_BUFFER, 
            3 * checkImageWidth * checkImageHeight, checkImage,
            GL_STATIC_DRAW);
}
...
void display(void)
{
   ...
   glRasterPos2i(0, 0);
   glBindBuffer(GL_PIXEL_UNPACK_BUFFER, pixelBuffer); // NEW
   glDrawPixels(checkImageWidth, checkImageHeight, GL_RGB,
                GL_UNSIGNED_BYTE, BUFFER_OFFSET(0)); // MODIFY
   glFlush();
}

...
struct file_operations scullp_fops = {
    ...
    .write =     scullp_write,
    .ioctl =     scullp_ioctl,
    .mmap =	 scullp_mmap, /* 用于将设置的内存映射到进程的地址空间 */
    .open =	 scullp_open,
    .release =   scullp_release,
    .aio_read =  scullp_aio_read,  /* 异步的读操作 */
    .aio_write = scullp_aio_write, /* 异步的写操作 */
};
...
int scullp_init(void)
{
     ...
     */
    scullp_devices = kmalloc(scullp_devs*sizeof (struct scullp_dev),
            GFP_KERNEL);
    if (!scullp_devices) {
        result = -ENOMEM;
        goto fail_malloc;

...
bk_svn_root_dir="/home/scr/host2vm/trunk"
add_list_dir="/home/scr/bin/hosts2vm"
...
tmpdir="/tmp"
tmpfile1="$tmpdir/bk_nal_tmp"
tmpfile2="$tmpdir/bk_nal_tmp2"
...
if [ -e "$1" ]; then	# 该文件或目录存在?
    if [ -f "$1" ]; then # 是文件?
	head -n 2 $1 > $tmpfile1
	tac $tmpfile1 > $tmpfile2
	flag_line=`head -n 1 $tmpfile2`

...
# file_integrity.sh: 检查一个给定目录下的文件
#                    是否被改动了.

case "$1" in "" | "-h" | "--help") 
echo "Usage: ${0##*/}";
echo "Example 12-34. 检查文件完整性";
echo "md5sum"
exit 65;;  # 没有命令行参数,
esac
...
check_database ()
{
    ...
    while read record[n] # 把dbfile文件读入数组record中, 以行为单位 
    do
	directory_checked="${record[0]}" # record[0]表示第1行 
	...
	then
	    ...
	    if [ "${record[n]}" = "${checksum[n]}" ]; then # record[n]表示第n行 
		echo "${filename[n]} unchanged."
	    elif [ "`basename ${filename[n]}`" != "$dbfile" ]; then
		#  跳过checksum 数据库文件,
		#+ 因为在每次调用脚本它都会被修改.
		#  ---
		...
		#+ 据库文件将不会被检测出来.
		#  练习: 修复这个问题.
		echo "${filename[n]} : CHECKSUM ERROR!" #echo $n
		# 因为最后的检查, 文件已经被修改.
	    fi
	fi

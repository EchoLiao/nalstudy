...
PID_FILE=/var/run/${0##*/}.pid # ${0##*/} 表示当前文件名(不包含路径)
...
init () {
    ...
    trap "{  
        rm -f $TMP 2>/dev/null
    }" EXIT          # 注册信号, 若收到任何信号则删除存在的临时文件.
}


check_pid () {
# 检查进程是否存在.
    if [ -s "$PID_FILE" ]; then  # -s: 文件大小不为零
        echo "PID file exists. Checking ..."
        PID=$(/bin/egrep -o "^[[:digit:]]+" $PID_FILE)
        if /bin/ps --pid $PID &>/dev/null; then
            echo "Process $PID found. ${0##*/} seems to be running!"
           /usr/bin/logger -t ${0##*/} \
                 "Process $PID found. ${0##*/} seems to be running!" # 写入系统日志 
            exit $E_RETURN
        fi

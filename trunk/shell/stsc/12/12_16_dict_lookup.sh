...
#
#  在通过本脚本使用之前,
#+ 先要将这本字典由 DOS 格式转换为 UNIX格式(只以 LF 作为行结束符).
#  将这个文件存储为纯文本形式, 并且保证是未压缩的 ASCII 格式.
#  将DEFAULT_DICTFILE 变量以 path/filename 形式设置好.
...
if [ -z "$2" ]       #  也可以指定不同的词典作为这个脚本的第2个参数传递进来.
then
...
#  从输出的第一行打印到下一部分的第一行.
sed '$d' | sed '$d'
#  删除输出的最后两行Delete last two lines of output
#+ (空行和下一部分的第一行).
# ---------------------------------------------------------
